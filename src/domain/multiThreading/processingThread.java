...
public class processingThread implements Runnable{
	...
	private void detectAndReportAlignment(ArrayList<GenomicLocation> potentialLocations, String readID, String read, 
		...
		if (maximumsCounter==1 && maximalScore>=0.9){			
	...
	private static AlignmentsData align(String readLeftFlank, String readRightFlank, String readRepetitiveRegion, String genomicLeftFlank, 
		...
		int totalForwardFlanksLength = leftSideForward.getLength()+ rightSideForward.getLength();
		int totalForwardFlanksIdenticals = leftSideForward.getNumIdenticals()+ rightSideForward.getNumIdenticals();

		int totalReverseFlanksLength = leftSideReverse.getLength()+ rightSideReverse.getLength();
		int totalReverseFlanksIdenticals = leftSideReverse.getNumIdenticals()+ rightSideReverse.getNumIdenticals();
		...
		double forwardScoreMin = Math.min((double)totalForwardFlanksIdenticals/totalForwardFlanksLength,(double)totalPairedEndRightIdenticals/totalPairedReadLength);
		double reverseScoreMin = Math.min((double)totalReverseFlanksIdenticals/totalReverseFlanksLength ,(double)totalPairedEndLeftIdenticals/totalPairedReadLength);
